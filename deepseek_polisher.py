"""DeepSeek-based academic polishing pipeline with paragraph-level reporting."""
...
class DeepSeekAcademicPolisher:
    """Run paragraph-level evaluation + two-pass polishing with DeepSeek API."""
    ...
    def _chat_with_retry(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                text = self._single_chat_call(system_prompt, user_prompt)
                if not text.strip():
    ...
    def _single_chat_call(self, system_prompt: str, user_prompt: str) -> str:
        body: Dict[str, Any] = {
            "model": self.model,
        ...
        if not choices:
            raise RuntimeError(f"API 返回缺少 choices: {data}")
